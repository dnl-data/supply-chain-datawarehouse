...
SELECT DISTINCT
    ...
    END AS month_name,
    CASE 
        WHEN EXTRACT(MONTH FROM snapshot_date) IN (1,2,3) THEN 1
        WHEN EXTRACT(MONTH FROM snapshot_date) IN (4,5,6) THEN 2
        WHEN EXTRACT(MONTH FROM snapshot_date) IN (7,8,9) THEN 3
        ELSE 4
    END AS quarter
FROM read_csv('s3://supply-chain-dwh/test/silver/silver_daily_inventory.csv', AUTO_DETECT=TRUE)
...
SELECT 
    ...
    is_active,
    is_discontinued
FROM read_csv('s3://supply-chain-dwh/test/silver/silver_products.csv', AUTO_DETECT=TRUE);
...
SELECT 
    ...
    di.zalora_allocated
FROM read_csv('s3://supply-chain-dwh/test/silver/silver_daily_inventory.csv', AUTO_DETECT=TRUE) di;
...
SELECT DISTINCT
    ...
    CASE channel
        ...
        ELSE 'Other'
    END AS channel_type
FROM read_csv('s3://supply-chain-dwh/test/silver/silver_sales.csv', AUTO_DETECT=TRUE)
...
SELECT 
    ...
    p.category,
    p.brand
FROM read_csv('s3://supply-chain-dwh/test/silver/silver_sales.csv', AUTO_DETECT=TRUE) s
LEFT JOIN read_csv('s3://supply-chain-dwh/test/silver/silver_products.csv', AUTO_DETECT=TRUE) p 
...
SELECT 
    ...
    status,
    lead_time_category
FROM read_csv('s3://supply-chain-dwh/test/silver/silver_suppliers.csv', AUTO_DETECT=TRUE);
...
SELECT 
    ...
    (po.delivery_date - po.po_date) AS lead_time_days,
    (po.promised_delivery_date - po.po_date) AS promised_lead_time_days,
    CASE 
    ...
    p.product_name,
    p.category
FROM read_csv('s3://supply-chain-dwh/test/silver/silver_purchase_orders.csv', AUTO_DETECT=TRUE) po
LEFT JOIN read_csv('s3://supply-chain-dwh/test/silver/silver_suppliers.csv', AUTO_DETECT=TRUE) s 
