-- GOLD LAYER : Data Marts with Foreign Keys
-- Read from S3 (silver layer) and write to S3 & local

-- =====================================================
-- DATA MART 1 : Stock & Inventory
-- =====================================================
...
FROM read_csv('s3://ton-bucket/silver/daily_inventory.csv', AUTO_DETECT=TRUE)
WHERE snapshot_date IS NOT NULL;

ALTER TABLE gold_stock.dim_date ADD PRIMARY KEY (date);
...
SELECT 
    ...
    is_active,
    is_discontinued
FROM read_csv('s3://ton-bucket/silver/products.csv', AUTO_DETECT=TRUE);

ALTER TABLE gold_stock.dim_product ADD PRIMARY KEY (sku_id);

-- fact_inventory with FKs
...
SELECT 
    ...
    di.zalora_allocated
FROM read_csv('s3://ton-bucket/silver/daily_inventory.csv', AUTO_DETECT=TRUE) di;

ALTER TABLE gold_stock.fact_inventory ADD FOREIGN KEY (snapshot_date) REFERENCES gold_stock.dim_date(date);
ALTER TABLE gold_stock.fact_inventory ADD FOREIGN KEY (sku_id) REFERENCES gold_stock.dim_product(sku_id);
...
ALTER TABLE gold_stock.alert_reorder ADD FOREIGN KEY (snapshot_date) REFERENCES gold_stock.dim_date(date);
ALTER TABLE gold_stock.alert_reorder ADD FOREIGN KEY (sku_id) REFERENCES gold_stock.dim_product(sku_id);

-- =====================================================
-- DATA MART 2 : Sales Department
-- =====================================================
...
SELECT DISTINCT
    ...
    CASE channel
        ...
        ELSE 'Other'
    END AS channel_type
FROM read_csv('s3://ton-bucket/silver/sales.csv', AUTO_DETECT=TRUE)
WHERE channel IS NOT NULL;

ALTER TABLE gold_sales.dim_channel ADD PRIMARY KEY (channel);
...
ALTER TABLE gold_sales.dim_promo ADD PRIMARY KEY (event_name);
...
SELECT 
    ...
    p.category,
    p.brand
FROM read_csv('s3://ton-bucket/silver/sales.csv', AUTO_DETECT=TRUE) s
LEFT JOIN read_csv('s3://ton-bucket/silver/products.csv', AUTO_DETECT=TRUE) p 
    ON s.sku_id = p.sku_id;

ALTER TABLE gold_sales.fact_sales ADD FOREIGN KEY (date) REFERENCES gold_stock.dim_date(date);
ALTER TABLE gold_sales.fact_sales ADD FOREIGN KEY (sku_id) REFERENCES gold_stock.dim_product(sku_id);
ALTER TABLE gold_sales.fact_sales ADD FOREIGN KEY (channel) REFERENCES gold_sales.dim_channel(channel);
ALTER TABLE gold_sales.fact_sales ADD FOREIGN KEY (event_name) REFERENCES gold_sales.dim_promo(event_name);
...
SELECT 
    ...
    status,
    lead_time_category
FROM read_csv('s3://ton-bucket/silver/suppliers.csv', AUTO_DETECT=TRUE);

ALTER TABLE gold_procurement.dim_supplier ADD PRIMARY KEY (supplier_id);

-- fact_purchase with FKs
...
SELECT 
    ...
    p.product_name,
    p.category
FROM read_csv('s3://ton-bucket/silver/purchase_orders.csv', AUTO_DETECT=TRUE) po
LEFT JOIN read_csv('s3://ton-bucket/silver/suppliers.csv', AUTO_DETECT=TRUE) s 
    ON po.supplier_id = s.supplier_id
LEFT JOIN read_csv('s3://ton-bucket/silver/products.csv', AUTO_DETECT=TRUE) p 
    ON po.sku_id = p.sku_id;

ALTER TABLE gold_procurement.fact_purchase ADD FOREIGN KEY (sku_id) REFERENCES gold_stock.dim_product(sku_id);
ALTER TABLE gold_procurement.fact_purchase ADD FOREIGN KEY (supplier_id) REFERENCES gold_procurement.dim_supplier(supplier_id);
ALTER TABLE gold_procurement.fact_purchase ADD FOREIGN KEY (po_date) REFERENCES gold_stock.dim_date(date);
